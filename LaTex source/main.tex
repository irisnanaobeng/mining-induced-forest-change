...
I would also like to warmly thank my course mate, Sofia Bissoni, for her support and encouragement during the Spatial Ecology course. Her willingness to share knowledge and provide guidance greatly enriched my learning experience and gave me the confidence to engage fully with the course and undertake the examination. Through that experience, I was able to further develop my interest in spatial ecology and demonstrate my commitment to the subject.
...
The results showed a progressive decline in vegetation condition in the study area, with the largest reductions observed between 2020 and 2022. NDVI change detection indicated that approximately 29.3\% of the study area experienced vegetation decline relative to 2018 baseline levels, while only 1.7\% showed vegetation recovery. Spectral distance analysis further revealed progressive divergence from the 2018 forest reference condition in areas surrounding mining-influenced zones, suggesting increasing ecological degradation over time.

The findings suggest that combining NDVI with spectral distance analysis may provide a more comprehensive approach for detecting both direct forest loss and subtle changes in vegetation condition than single-index approaches alone. The study contributes to understanding the environmental change related to mining in Ghana and demonstrates the usefulness of integrated remote sensing methods to monitor forest degradation in tropical landscapes. Limitations include the absence of field validation; results should therefore be interpreted as relative patterns rather than absolute measurements.
...
Spectral distance analysis is a remote sensing technique used to evaluate the similarity or dissimilarity between spectral signatures. The technique is based on the assumption that different land-cover types and surface conditions exhibit unique spectral signatures across multiple wavelength bands, allowing them to be distinguished using remotely sensed imagery \citep{richards2013remote}.
...
Previous studies have applied spectral distance analysis to monitor forest degradation associated with logging, agricultural expansion, fire, and mining activities \citep{tarazona2020monitoring}. Combining spectral distance analysis with NDVI may therefore improve the detection of both direct vegetation loss and progressive ecological degradation.
...
A rule-based classification procedure was developed to distinguish major land-cover types within the Pra River Basin. The classification was based on NDVI threshold values derived from commonly reported vegetation characteristics in tropical environments and iteratively refined through visual comparison with Sentinel-2 true-colour and false-colour composite images. Threshold values were selected to maximize the separation between healthy vegetation, disturbed vegetation, bare soil, and water bodies.
...
Mining-related disturbance was identified using a change-based NDVI approach. Pixels exhibiting high NDVI values in 2018 (NDVI $>$ 0.45), representing healthy vegetation, and low NDVI values in 2022 (NDVI $<$ 0.30), indicating vegetation removal or severe disturbance, were classified as areas of likely mining-related vegetation loss.

This classification represents a change-detection indicator derived from NDVI differences between 2018 and 2022 rather than a direct land-cover class. The approach is based on the assumption that substantial vegetation loss within the study area is likely associated with mining-related land disturbance. By considering changes in vegetation condition over time, the method helps distinguish areas of recent vegetation loss from stable non-vegetated surfaces and other naturally occurring bare land-cover types. Similar NDVI-based approaches have been applied in studies of mining-affected landscapes in Ghana \citep{barenblitt2021ghana}.
...
The analysis used four Sentinel-2 spectral bands (blue, green, red, and near-infrared), which were combined into a multi-band spectral stack for each year. To reduce computational demand, the spectral data were aggregated from 10 m to 100 m spatial resolution. This aggregation reduces spatial detail and may smooth small-scale mining features; however, it was necessary for computational feasibility given the study area extent.
...
This study does not include independent validation data. No field surveys or higher-resolution reference imagery were available to assess classification accuracy. Consequently, the land-cover and change-detection results presented in Chapter 5 represent relative patterns rather than validated measurements. Uncertainty in NDVI threshold selection, spectral centroid identification, and cloud masking may affect the results. These limitations are addressed further in Section 6.6.
...
\begin{figure}[H] \centering \includegraphics[width=0.85\textwidth,height=0.55\textheight,keepaspectratio]{maps/vegetation_mining_2022.png} \caption{NDVI-based land-cover classification showing mining disturbance areas in 2022.} \label{fig:classification_2022} \end{figure}
...
In 2018, most pixels exhibited relatively large distances from the mining centroid, indicating that mining disturbance surfaces occupied only a limited proportion of the study area. By 2020, the distribution shifted slightly toward lower distances, suggesting an increase in mining disturbance-related surfaces. The strongest shift occurred in 2022, when a larger proportion of pixels exhibited low spectral distance to the mining centroid. This suggests an increase in mining-related spectral characteristics across the landscape and is consistent with the expansion of mining disturbance during the study period.
...
\begin{figure}[H] \centering \includegraphics[width=\textwidth,height=0.5\textheight,keepaspectratio]{maps/forest_spectral_analysis.png} \caption{Spectral distance of landscape pixels to the forest centroid between 2018 and 2022.} \label{fig:forest_spectral} \end{figure}
...
These findings indicate a spatial association between mining disturbance, vegetation decline, and broader patterns of forest degradation across the surrounding landscape. The combined use of NDVI and spectral distance analysis therefore provided complementary evidence of both land-cover change and gradual ecological deterioration within the study area.
...
The mapped mining disturbance areas revealed that many sites were located within or adjacent to previously vegetated areas. Although vegetation loss within the Pra River Basin may also be influenced by agricultural expansion, settlement development, and other land-use changes, mining-related disturbance was considered the most likely driver of the observed degradation patterns. This interpretation is supported by the known concentration of artisanal and small-scale mining activities within the basin, the spatial clustering of detected disturbance zones within mining-influenced landscapes, and the close correspondence between mining disturbance areas, bare soil surfaces, and vegetation decline identified through NDVI change detection.

Consequently, while the analysis cannot attribute all observed vegetation loss exclusively to mining activities, the spatial evidence suggests that mining-related land disturbance represents a major contributor to the observed patterns of forest degradation. Areas surrounding these clusters also exhibited lower NDVI values and greater spectral divergence from the healthy forest reference condition. This indicates that the environmental effects of mining disturbance extend beyond the immediate excavation sites.
...
The density distributions indicated that the spectral properties of the landscape became progressively less similar to the 2018 forest reference condition over time. Forest spectral distances exhibited a clear temporal shift toward larger values between 2018 and 2022, indicating growing divergence from healthy vegetation signatures.
...
First, the study relied entirely on satellite-based analysis and did not include field validation. Consequently, the classification results represent relative patterns of degradation rather than direct ecological measurements, and classification uncertainty may still affect interpretation of land-cover transitions \citep{foody2002status}. Future studies should incorporate field observations or independent validation datasets to assess classification accuracy and improve confidence in the identification of mining disturbance and vegetation degradation classes.
