...
title: "Mining-Induced Forest Degradation in Ghana’s Pra River Basin: A Multi-Temporal Remote Sensing Analysis"
...
knitr::opts_chunk$set(echo = TRUE, warning = FALSE, message = FALSE, fig.path = "maps/")


# Consistent color palette for all plots
palette <- list(
  mining = "#D73027",               # deeper mining red
  healthy_forest = "#1B7837",      # strong forest green
  disturbed_forest = "#FFC125",    # goldenrod1
  bare_soil = "#C7A76C",           # earthy brown
  water_bodies = "#4F94CD",        # steel blue for water
  no_change = "#FFEFD5",           # Cornsilk - no change
  decrease = "#B2182B",            # strong red for NDVI decrease
  increase = "#1A9850",            # strong green for NDVI increase
  background = "#FFBBFF",          # Linen - warm background
  study_area = "#9B30FF",          # Medium purple - study area highlight
  ghana_bg = "#FFE4B5"             # Seashell - Ghana background
)
...
Sentinel-2 imagery for PRA River Basin was acquired from the
...
All Sentinel-2 imagery was preprocessed prior to analysis. 
Bands were reprojected to a common coordinate reference system (CRS), 
spatially aligned, and clipped to the Pra River Basin boundary. 
Cloud-contaminated pixels were excluded where necessary.
...
combined_true_color
ggsave("maps/true_color_comparison.png", combined_true_color, 
...
ggsave("maps/false_color_comparison.png", combined_false_color, 
       width = 18, height = 6, dpi = 300, bg = "white")
...
p_ndvi_diff_18_22 <- plot_singleband_gg(ndvi_diff_18_22, title = "NDVI Difference (2022-2018)") +
  scale_fill_gradient2(
    low = palette$decrease,      # Indian red
    mid = palette$no_change,     # Cornsilk
    high = palette$increase,     # Medium sea green
    midpoint = 0, 
    name = "ΔNDVI"
  )
...
p_ndvi_change_18_22_class <- ggplot(df_change, aes(x = x, y = y, fill = class)) +
  ...
  scale_fill_manual(
    values = c(
      ...
      "Vegetation Decrease" = palette$decrease 
    ),
    name = "NDVI Change",
    na.value = "white"
  ) +
...
combined_ndvi_change <- (p_ndvi_diff_18_22 | p_ndvi_change_18_22_class) +
  plot_annotation(
    title = "NDVI Change Detection Pra River Basin (2018–2022)",
    theme = theme(plot.title = element_text(hjust = 0.5, size = 16, face = "bold"))
  ) &
  theme(plot.margin = margin(5, 5, 5, 5))
...
# --- Aggregate classification and NDVI ---
class_small <- terra::aggregate(vegetation_classes, fact = 8, fun = modal, na.rm = TRUE)
...
# --- Extract ONLY mining pixels (class = 2) ---
df_mining_points <- df_class[df_class$class == 2, ]

# --- Thin points (prevents overplotting = cleaner map) ---
set.seed(42)
...
p_mining <- ggplot() +
  ...
  # Background NDVI
  geom_raster(data = df_ndvi,
              aes(x = x, y = y, fill = ndvi)) +
  
  scale_fill_viridis_c(option = "D", name = "NDVI") +
  
  # Mining points overlay
  geom_point(data = df_mining_points,
             aes(x = x, y = y),
  ...
  labs(
    ...
    subtitle = "Red dots represent mining pixels extracted from classification (Class 2)",
    x = "Longitude",
    y = "Latitude"
  ) +
...
ggsave("maps/mining_distribution_dots_2022.png", p_mining,
